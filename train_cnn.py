...
X_s = X_init# [:n] #Added to cut down run time
y_s = y_init# [:n] #This too

X_small= []
y_small = []

print(y_s.shape, y_s.min(), y_s.max())

print("max z from 0 to 0.2")
j = 0
...
for i in range(len(y_s)):
	...
	if y_s[i,:] <= 0.2:
	    if y_s[i,:]>0:
   	        y_small.append(y_s[i,:])
	        X_small.append(X_s[i,:,:,:])
	        j+=1
    else:
	break

y_small = list(map(np.float32, y_small))
X_small = list(map(np.float32, X_small))	

y_small = np.array(y_small)
X_small = np.array(X_small)
n = len(y_small)

print(len(y_small), y_small.shape, y_small.min(), y_small.max())
...
print("X_init.shape = {}, X_init.min = {}, X_init.max = {}".format(X_sml.shape, X_sml.min(), X_sml.max()))

print("y_init.shape = {}, y_init.min = {}, y_init.max = {}".format(y_small.shape, y_small.min(), y_small.max()))
