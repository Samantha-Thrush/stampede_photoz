...
# THESE HELP THE FUNCTIONALITY OF DOING COLORS AND SUB-BANDS

n = 45000
color = 4
bands = 'ugri'  # Please enter as a string: 'ugriz' or 'u-i'
subtr = FALSE
...
def lsrange(bands, subtr):
    if !subtr:
	bandls = []
    	for i in bands:
            bandls.append(int(transband(i)))
        return(array(banidls))
    else:
	first = int(transband(bands[0]))
	second = int(transband(bands[2]))
	return(first, second)
...
if subtr:
    fst, scnd = lsrange(bands, subtr)
    X_sml[:,0,:,:] = renormalize(X_small[:,fst,:,:]-X_small[:,scnd,:,:])
  
else:
    for i in lsrange(bands, subtr):
